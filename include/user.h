...
void clock()
{
    ...
    bool endofmin=false;
    int minNum=0;
    Window clock("Clock", 20, 25, 75, 35);
    drawObj(&clock);
    char c;
...
Window wnd("Space Invaders", WCENTERX, WCENTERY, WHEIGHT, WWIDTH);
//DRAW FUNCTIONS
...
void textEdit()
{
	...
	bool caps = false;
    char c;
    Window editor("Text Editor Proto", 10, 10, 1000, 600);
    drawObj(&editor);
	//spaceInv();
...
void about()
{
    ...
    drawmain();
    //drawWallpaper();
    Window info("OS Info", 40, 30, 540, 80);
	drawObj(&info);
...
void minesweeper()
{
	...
	int posx=0, posy=0;
	drawmain();
    Window mswp("mswp", 100, 100, 225, 226);
	drawObj(&mswp);
...
void pixArt()
{
    ...
    bool erase=false;
    Window part("pixArt", 100, 100, 910, 550);
	drawObj(&part);
	again:
...
void authors()
{
    ...
    //gets(0,0);
    //drawWallpaper();
    Window authors("Authors", 40, 30, 500, 120);
    drawObj(&authors);
...
void wallColor()
{
    ...
    drawmain();
    //drawWallpaper();
    Window wcolor("Wallpaper", 20, 20, 430, 40);
	drawObj(&wcolor);
...
void graphicsEdit()
{
	...
	drawmain();
	//drawWallpaper();
	Window os365brush("Graph", 20, 50, 1000, 600);
	drawObj(&os365brush);
	word x = 0;
	word y = 0;
	uint8_t color=0;
...
void help()
{
    ...
    drawmain();
    Window help("Help", 10, 30, 1000,500);
	drawObj(&help);
	int i;
